...
graph TD
    ...
    D -->|Arquivo de Saída| E[automations_gerado.yaml]
```
...
* **`_parsear_tempo(tempo_str)`** (Linha 42):
  * **O que faz:** O tradutor de grandezas temporais. Ele usa Expressões Regulares (`re.match`) para decodificar strings simplificadas em dicionários de atraso (`delay`) nativos do Home Assistant.
  * **Regras de conversão:**
    * `'10s'` $\rightarrow$ `{'hours': 0, 'minutes': 0, 'seconds': 10, 'milliseconds': 0}`
    * `'5min'` $\rightarrow$ `{'hours': 0, 'minutes': 5, 'seconds': 0, 'milliseconds': 0}`
    * `'01:30:00'` $\rightarrow$ `{'hours': 1, 'minutes': 30, 'seconds': 0, 'milliseconds': 0}`

### B. Métodos de Mapeamento Estrutural (AST $\rightarrow$ HA Schema)

* **`_gerar_automacao(self, no)`** (Linha 106):
  * Cria o esqueleto base de uma automação conforme o padrão oficial exigido pelo parser de automações do Home Assistant:
    ...
    * `mode`: configurado estaticamente como `'single'`.

* **`_gerar_triggers(self, gatilhos)`** (Linha 122):
...
* **`_gerar_actions(self, acoes)`** (Linha 167):
...
### C. Serialização YAML (Linhas 207-227)
...
Uma string formatada contendo a listagem serializada gravada no arquivo `automations_gerado.yaml`:
```yaml
- id: '1685712495000'
  alias: Por do sol
  description: ''
...
* **O que o professor pode pedir:** *"Quero que a linguagem passe a aceitar horas no formato de delay das ações (ex: `2h`). Mude a geração de código para dar suporte a isso."*
* **Como resolver ao vivo:**
  Você só precisaria adicionar uma nova cláusula de regex no método `_parsear_tempo` (Linha 42) para extrair as horas:
  ```python
  # Formato: Nh (horas)
  match_h = re.match(r'^(\d+)h$', tempo_str)
  if match_h:
      return {
  ...
  ```
  *(Esta pequena inserção de 7 linhas garante o mapeamento completo e seguro de horas para o delay do Home Assistant!)*
...
2. **Execute a Geração:** A seção `if __name__ == '__main__':` (Linha 234) executa toda a cadeia de tradução e gera o arquivo `automations_gerado.yaml` no disco automaticamente. Execute:
   ```bash
   python codegen.py
   ```
