...
1. **Validação Gramatical:** Ele recebe o fluxo linear de tokens do `lexer.py` e valida se a sequência obedece à **Gramática Livre de Contexto (GLC)** definida para a linguagem Homi (por exemplo: se toda automação declarada possui a palavra-chave `AUTOMACAO`, seguida por um nome em string, blocos válidos de `GATILHO:`, `ACAO:` e fecha com a palavra-chave `FIM`).
...
* **Regra Principal da Automação (`p_automacao`):**
  ...
  ```
  * **O que faz:** Esta é a regra estrutural principal. Ela consome os blocos lógicos da automação e encapsula tudo em um dicionário estruturado que servirá de nó para a AST.
...
* **Regra de Condição Vazia (`p_bloco_condicao_vazio`):**
  ...
  def p_bloco_condicao_vazio(p):
      ...
      p[0] = []
  ```
  * **O que faz:** Esta regra de derivação vazia ($\epsilon$) torna a seção de condições opcional. Se a palavra-chave `CONDICAO` não estiver no código da automação, a AST de condições receberá uma lista vazia `[]` sem estourar erro sintático.

* **Regras de Gatilhos e Ações:**
  * As regras de `comando_gatilho` (Linhas 76-92) mapeiam gatilhos de tempo (`QUANDO sunset -01:30:00`) ou gatilhos de estado de entidade (`QUANDO light.sala ESTA "on"`).
  * As regras de `comando_acao` (Linhas 168-190) mapeiam ações de ligar, desligar e esperar (`ESPERAR 5min`). Cada um desses retorna um dicionário descrevendo especificamente a operação.

### C. Sistema de Recuperação de Erros (Modo Pânico)

O seu parser possui uma estratégia de resiliência muito sofisticada baseada em **sincronização no Modo Pânico**:

1. **`p_automacao_erro(p)`** (Linha 198):
...
* **Onde pode ocorrer:** Atualmente, a gramática é livre de conflitos porque cada bloco possui delimitadores exclusivos (`GATILHO:`, `CONDICAO:`, `ACAO:`). Mas se você remover a obrigatoriedade desses cabeçalhos ou adicionar aninhamentos de expressões condicionais sem escopo explícito, conflitos LALR aparecerão.
